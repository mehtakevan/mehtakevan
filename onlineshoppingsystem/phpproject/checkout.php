...
<header>
<blockquote >
	<img src="images/logonew.jpg" height="70px" >
...
if(isset($_SESSION['id'])){
	...
	$result = $conn->query($sql);
	while($row = $result->fetch_assoc()){
		$sql = "INSERT INTO `order`(CustomerID, MobileID, DatePurchase, Quantity, TotalPrice, Status) 
		VALUES(".$row['CustomerID'].", '".$row['MobileID']
	...
	$sql = "SELECT customer.CustomerName, customer.PinCode, customer.CustomerGender, customer.CustomerAddress, customer.CustomerEmail, customer.CustomerPhone, mobiles.ModelName, mobiles.Price, mobiles.Image, `order`.`DatePurchase`, `order`.`Quantity`, `order`.`TotalPrice`
		FROM customer, mobiles, `order`
		WHERE `order`.`CustomerID` = customer.CustomerID AND `order`.`MobileID` = mobiles.MObileID AND `order`.`Status` = 'N' AND `order`.`CustomerID` = ".$cID."";
	$result = $conn->query($sql);
...
<input class="button" style="float: right; margin: 2%;" type="button" name="submit" value="Place Order" onClick="window.location='Confirm.html';" />
<?php
	...
	while($row = $result->fetch_assoc()){
    	...
    	echo $row['ModelName']."<br>Rs.".$row['Price']."<br>";
...
$nameErr = $emailErr = $genderErr = $addressErr = $pincodeErr = $contactErr = "";
$name = $email = $gender = $address = $pincode = $contact = "";
$cID;
	...
	if (empty($_POST["name"])) {
		...
		if (!preg_match("/^[a-zA-Z ]*$/", $name)){
			...
			if (empty($_POST["pincode"])){
				$pincodeErr = "Please enter your Pin Code";
			}else{
				if(!preg_match("/^[0-9 -]*$/", $pincode)){
					$pincodeErr = "Please enter a valid Pin Code";
					$pincode = "";
				}else{
								...
								if(!preg_match("/^[0-9 -]*$/", $contact)){
									...
									if (empty($_POST["gender"])){
										...
										if (empty($_POST["address"])){
											...
											$sql = "INSERT INTO customer(CustomerName, CustomerPhone, PinCode, CustomerEmail, CustomerAddress, CustomerGender) 
											VALUES('".$name."', '".$contact."', '".$pincode."', '".$email."', '".$address."', '".$gender."')";
											$conn->query($sql);
 
											$sql = "SELECT CustomerID from customer WHERE CustomerName = '".$name."' AND PinCode = '".$pincode."'";
											$result = $conn->query($sql);
											while($row = $result->fetch_assoc()){
												$cID = $row['CustomerID'];
											}
...
if(!isset($_SESSION['id'])){
	...
	echo 'PIN Code:<br><input type="text" name="pincode" placeholder="xxxxxx">';
	echo '<span class="error" style="color: red; font-size: 0.8em;"><?php echo $pincodeErr;?></span><br><br>';
